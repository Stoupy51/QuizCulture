...
$nomLigue = ["OR III","OR II","OR I","ARGENT III","ARGENT II","ARGENT I","BRONZE III","BRONZE II","BRONZE I","MASTER"];
$ligueActuelle = "";
...
//Prendre les questions validées selon le niveau
$stmt = MyPDO::getInstance()->prepare(<<<SQL
    SELECT idQuestion
    FROM Question
    WHERE isVerified = 1
    AND niveau BETWEEN :min AND :max
SQL);
$stmt->execute([":min"=>$key-2,":max"=>$key+2]);
...
$affichage
        ...
        <div class="d-flex mb-5">
            ...
            <a class="mx-5 p-2 buttonsuivant" href="ligue.php"><svg width="25" height="25" viewBox="0 0 45 45" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
...
$html->appendContent(<<<HTML
    <script>
        ...
        var time = '$temps'; 
        document.write(' <?php UpdatePoints(); ?> ');
    </script>
    <script src="js/{$script}Ligue.js"></script>
HTML);
...
function UpPoints() {
    global $idJoueur;
    $stat = MyPDO::getInstance()->prepare(<<<SQL
        UPDATE Joueur SET points = points + 2;
        WHERE idJoueur = :id
SQL);
